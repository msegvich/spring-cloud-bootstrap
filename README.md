...
**Eclipse Project Setup:** gradlew eclipse

**Build Application:** gradlew

**Starting Spring Boot:** gradlew bootRun

**Running WAR from CMD:**
1. Confirm Java 8 is on your path
2. **Run:** java -Derac.serverLevel=desktop -DLOG\_LEVEL=debug -DLOG\_PATH=c:/builds/cloud-demo/logs -jar C:\builds\cloud-demo\libs\cloud-demo-{version}.war

**Build Location:** C:\builds\cloud-demo

**Logs:** C:\builds\cloud-demo\logs

**Health URL:** http://localhost:8080/cloud-demo/health
...
You'll also need to specify where your configuration lives.
-Dspring.cloud.config.server.native.searchLocations=${HOME}/Projects/cloud-demo/configuration

# Implementations

Being the default dev or a feature branch, it could be either Consul or Eureka depending on what's
been merged. Check the settings of the projects to ensure you're using the right version if pulling
...
## Root Project and legacy-services/legacy\_user\_service

These are the same project. It is an example of a simple SOAP based CRUD service for handling user
details. There is nothing new about this service, it is just a basic example of a Spring WS app
using SOAP.
...
## new-services/zuul-front

This project is a simple boilerplate Zuul implementation in Spring. Zuul automatically load balances
for a cloud/microservice architecture, and all that needs to be configured is how to actually route
requests, which can be done in the YAML or Java Properties Spring configurations. This is a webapp
...
aren't running with Consul.

## batch/update-legacy-db

This project is an example batch program utilizing Spring Batch that would run to push any new 
